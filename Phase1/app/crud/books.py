...
def create_book(db: Session, book: schemas.BookCreate):
    current_time = datetime.utcnow()  # This is already UTC but doesn't include timezone info
    db_book = models.Book(
        **book.model_dump(),
...
def update_book(db: Session, book_id: int, book_data: schemas.BookUpdate):
    ...
    db_book.updated_at = datetime.now(timezone.utc)  # Fixed here
    db.commit()
    db.refresh(db_book)
    return db_book
...
def update_copies(db: Session, book_id: int, delta: int):
    ...
    if book:
        ...
        book.updated_at = datetime.now(timezone.utc)  # Fixed here
        db.commit()
        db.refresh(book)
    return book
