...
async def health_check():
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        service_health_endpoints = {
            "users": f"{SERVICES['users']}/api/users/health",
            "books": f"{SERVICES['books']}/api/books/health",
            "loans": f"{SERVICES['loans']}/api/loans/health"
        }
...
async def users_docs():
    ...
    return RedirectResponse(url="http://localhost:8080/docs")

@app.get("/docs/books") 
async def books_docs():
    """Redirect to Book Service documentation"""
    return RedirectResponse(url="http://localhost:8082/docs")

@app.get("/docs/loans")
async def loans_docs():
    """Redirect to Loan Service documentation"""
    return RedirectResponse(url="http://localhost:8081/docs")
